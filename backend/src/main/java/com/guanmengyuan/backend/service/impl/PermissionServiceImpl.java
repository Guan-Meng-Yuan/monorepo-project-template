...
public class PermissionServiceImpl extends ServiceImpl<PermissionMapper, Permission>
        implements PermissionService, CommandLineRunner {

    @Transactional
    private void initPermission(String tenantId) {
        // 初始化常量路由（只初始化一次，不受租户限制）
        initConstantRoutes();
        // 初始化其他路由数据
        initRoutesFromJson(tenantId);
    }
    ...
    @Override
    @Transactional
    public void initTenantRoutes(String tenantId) {
        // 确保常量路由已初始化
        initConstantRoutes();
        // 检查该租户的路由是否已存在
    ...
    private void initConstantRoutes() {
        TenantManager.withoutTenantCondition(() -> {
            ...
            saveRoute(TenantConstant.CONSTANT_ROUTE_TENANT_ID, null, "login",
                    ...
                    "layout.blank$view.login", true, "login", "route.login", null, null, true, null, null,
                    "2", true);
     ...
     */
    @Transactional
    private void initRoutesFromJson(String tenantId) {
    ...
    public void run(String... args) throws Exception {
        TenantManager.withoutTenantCondition(() -> {
            if (!Permission.of().exists()) {
                initPermission(TenantConstant.DEFAULT_TENANT_ID);
                initRolePermission(TenantConstant.DEFAULT_TENANT_ID);
            }
        });
    }

    private void initRolePermission(String tenantId) {
        TenantManager.withoutTenantCondition(() -> {
            List<Long> permissionIds = Permission.of().select(Permission::getId).where(Permission::getTenantId)
                    .eq(tenantId)
